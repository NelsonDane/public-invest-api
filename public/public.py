...
class Public:
    ...
    def login(self, username = None, password = None):
        ...
        response = self.session.post(endpoints.login_url(), headers=headers, data=payload, timeout=self.timeout, cookies=cookies)
        if response.status_code != 200:
            raise Exception("Login failed, check credentials")
        response = response.json()
        if response["twoFactorResponse"] is not None:
            # self._clear_cookies()
            print(f'2FA required, code sent to phone number ending in {response["twoFactorResponse"]["phoneNumberLastFour"]}...')
            code = input("Enter code: ")
            payload = endpoints.build_payload(username, password, code)
            response = self.session.post(endpoints.mfa_url(), headers=headers, data=payload, timeout=self.timeout, cookies=cookies)
            if response.status_code != 200:
    ...
    def _refresh_token(self):
        headers = self.session.headers
        response = self.session.post(endpoints.refresh_url(), headers=headers, timeout=self.timeout)
        if response.status_code != 200:
    ...
    def get_portfolio(self):
        ...
        portfolio = self.session.get(endpoints.portfolio_url(self.account_uuid), headers=headers, timeout=self.timeout)
    ...
    def get_symbol_price(self, symbol):
        headers = endpoints.build_headers(self.access_token)
        response = self.session.get(endpoints.get_quote_url(symbol), headers=headers, timeout=self.timeout)
    ...
    @login_required
    def place_order(symbol, quantity, side, order_type, time_in_force, limit_price=None, stop_price=None):
